...
function placeShipHandler(length, x, y) {
  ...
  const placed = currentPlacingPlayer.board.placeShip(
    length,
    [x, y],
    orientation,
  );
  if (placed) {
    ...
    const allShipPlaced = checkPlacementShip(
      shipInDock,
      currentPlacingPlayer.board.numberOfShips,
    );
    if (allShipPlaced) {
...
function previewHandler(length, x, y) {
  const preview = currentPlacingPlayer.board.checkPlace(
    length,
    [x, y],
    orientation,
  );
...
function attackCell(targetPlayer, x, y) {
  ...
  if (targetPlayer.board.allShipSunk) {
    ...
    const resutDiv = renderTurnResult(
      resultText,
      () => showResultGame(),
      true,
      true,
  ...
  if (game.currentPlayer.isComputer) {
    ...
    // showPassScreen(game.currentPlayer)
    const resutDiv = renderTurnResult(resultText, () =>
      showPassScreen(game.currentPlayer),
    );
    mainDiv.append(resutDiv);
...
function showPlayerPlacement(board, playerName) {
  ...
  const boardPlayer = renderPlayerBoardWrapper(
    board,
    playerName,
    removeShipHandler,
    placeShipHandler,
...
function renderBattleView(viewingPlayer) {
  ...
  // render boards on page
  const boardViewingPlayer = renderPlayerBoardWrapper(
    viewingPlayer.board,
    viewingPlayer.name,
    (x, y) => attackCell(viewingPlayer, x, y),
  );
  const boardOpponent = renderPlayerBoardWrapper(
    opponent.board,
    opponent.name,
...
function showPassScreen(player) {
  ...
  const passeScreen = renderPassScreen(player.name, () =>
    renderBattleView(player),
  );
...
function showResultGame() {
  ...
  // render boards on page
  const boardPlayerOne = renderPlayerBoardWrapper(
    game.playerOne.board,
    game.playerOne.name,
    null,
  );
  const boardPlayerTwo = renderPlayerBoardWrapper(
