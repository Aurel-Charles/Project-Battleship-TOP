...
export function renderPlayerBoardWrapper(board, playerName, onCellClick, placeShipHandler, previewHandler) {
    ...
    const boardPlayer = renderBoard(board, playerName, onCellClick, placeShipHandler, previewHandler)
    wrapper.append(title,boardPlayer)
    return wrapper   
}

export function renderBoard(board, playerName, onCellClick, onCellDrop, previewHandler) {
    ...
    if (onCellDrop) {
        ...
        boardEl.addEventListener('dragenter', (e)=> {
            ...
            cellPreviewCoor.forEach(coor => {
                const cellToColor = boardEl.querySelector(`[data-x="${coor[0]}"][data-y="${coor[1]}"]`)
